...
CURL_OPTIONS="-s -u YOUR_HTTP_USER:YOUR_HTTP_PASS"
...
if [[ $NAGIOS_SERVICECHECKCOMMAND == check_graphite_metric* ]] ; then
	...
	GRAPH_BASE64=$(/usr/bin/curl ${CURL_OPTIONS} "${GRAPHITE_BASEURL}${GRAPHITE_METRIC}&from=-6hours" | /usr/bin/base64 -w0)
...
#echo "emaileando a ${EMAIL_TARGET}" >> /tmp/nagios.log
